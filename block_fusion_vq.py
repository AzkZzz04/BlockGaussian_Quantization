...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        for block_id in range(0, num_blocks):
            pcd_dir = os.path.join(cfg.output_dirpath, "point_cloud", str(block_id))
            if not os.path.isdir(pcd_dir):
                raise FileNotFoundError(f"Missing point_cloud dir: {pcd_dir}")

            ply_files = [f for f in os.listdir(pcd_dir) if f.lower().endswith(".ply")]
            if not ply_files:
                raise FileNotFoundError(f"No .ply found in {pcd_dir}")

            # 只选 original
            originals = [f for f in ply_files if "original" in f.lower()]
            if len(originals) == 0:
                originals = [f for f in ply_files if "quantized" not in f.lower()]
                if len(originals) == 0:
                    raise FileNotFoundError(f"No suitable original .ply found in {pcd_dir}")

            optimized_pcd_filename = max(
                originals,
                key=lambda f: os.path.getmtime(os.path.join(pcd_dir, f))
            )
            optimized_pcdpath = os.path.join(pcd_dir, optimized_pcd_filename)
            ...
            print("block_id:", block_id, ", num pts before postfix:", mask_position.shape[0],
                  ", num pts after postfix:", mask_position.sum().item())
            ...
            block_gaussian.save_ply(os.path.join(cfg.output_dirpath, "point_cloud_postfix",
                                                 "point_cloud_{}.ply".format(block_id)))
    ...
    if args.merge:
        ...
        ply_filepaths = []
        for block_id in range(0, num_blocks):
            ply_filepaths.append(os.path.join(cfg.output_dirpath, "point_cloud_postfix",
                                              "point_cloud_{}.ply".format(block_id)))
